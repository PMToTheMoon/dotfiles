...
class DateEditingController extends GuideEditingController {
  ...
  DateEditingController.yM({DateTime? date})
      : format = DateFormat('MM/y'),
        super(
          guide: 'MM/AAAA',
          text: date != null ? DateFormat('MM/y').format(date) : null,
        );
...
class _DatePickerFieldState extends State<DatePickerField> {
  ...
  DateTime get _lastDate => widget.lastDate ?? DateTime(2200);

  DateTime get _initialDate {
    final initDate = _controller.date ?? widget.initialDate;
    if (initDate != null &&
        initDate.isAfter(_firstDate) &&
        initDate.isBefore(_lastDate)) {
      return initDate;
    }
    return DateTime.now();
  }
