...
import 'package:zanalys/authentication/otp/bloc/otp_event.dart';
import 'package:zanalys/authentication/otp/bloc/otp_state.dart';
...
class OTPBloc extends Bloc<OTPEvent, OTPState> {
  ...
  void _onOTPSubmitted(OTPSubmitted event, Emitter<OTPState> emit) async {
    ...
    if (state.status.isValidated) {
      ...
      try {
        Session? session = await AppServices.network
            .validateOTP(identifier: identifier, password: password, code: otp);
        if (session != null && session.token.isNotEmpty) {
          await AppServices.storage.saveToken(session.token);
          await AppServices.storage.saveRefreshToken(session.refreshToken);
          await AppServices.storage.saveUser(session.user);
  ...
  _sendPushToken() async {
    try {
      ...
      if (token != null) {
        await AppServices.network.updatePushToken(token: token);
      }
    } catch (e) {
      print(e);
    }
