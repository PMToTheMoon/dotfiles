...
class _FlightResult extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (state is FlightSelectionSuccess) {
      ...
      if (state.airport?.label != null) {
        label = context.l10n.flight_found_message_full(
            state.airport?.label ?? "",
            date.dateDisplay(context),
            date.timeDisplay(context));
      } else {
        label = context.l10n.flight_found_message(
            date.dateDisplay(context), date.timeDisplay(context));
      }
      return Padding(
        padding: const Padding4(),
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            context.subtitle2("Arrivé prévue"),
            const Space5.h(),
            Row(
              children: [
                const Icon(
                ...
                const Space5.w(),
                Text(date.fullDateTimeDisplay(context))
              ],
            ),
            const Space5.h(),
            Row(
