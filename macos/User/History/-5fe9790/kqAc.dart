...
/// Collects the expected AssetIds created by [generator] when given [input] based
/// on the extension configuration.
Iterable<AssetId> expectedOutputs(Generator generator, AssetId input) sync* {
  for (var parsedExtension in generator._parsedExtensions) {
    var outputs = parsedExtension.matchingOutputsFor(input);
    for (var output in outputs) {
      if (output == input) {
        throw ArgumentError(
          'The generator `$generator` declares an output "$output" which is '
...
// We can safely cache parsed build extensions for each generator since build
// extensions are required to not change for a generator.
final _parsedInputs = Expando<List<_ParsedBuildOutputs>>();

/// Extensions on [Generator] describing expected outputs.
extension BuildOutputExtensions on Generator {
  /// Whether this generator is expected to output assets when running on [input].
  ///
...
abstract class _ParsedBuildOutputs {
  ...
  factory _ParsedBuildOutputs.parse(
      Generator generator, String input, List<String> outputs) {
    final matches = _captureGroupRegexp.allMatches(input).toList();
    if (matches.isNotEmpty) {
      return _CapturingBuildOutputs.parse(generator, input, outputs, matches);
    }
    ...
    for (final output in outputs) {
      if (_captureGroupRegexp.hasMatch(output)) {
        throw ArgumentError(
          'The generator `$generator` declares an output "$output" using a '
...
class _CapturingBuildOutputs extends _ParsedBuildOutputs {
  ...
  factory _CapturingBuildOutputs.parse(Generator generator, String input,
    ...
    // Generators can have multiple capture groups, which are disambiguated by
    ...
    for (final match in matches) {
      ...
      if (names.contains(name)) {
        throw ArgumentError(
          'The generator `$generator` declares an input "$input" which contains '
    ...
    for (final output in outputs) {
      ...
      for (final outputMatch in _captureGroupRegexp.allMatches(output)) {
        ...
        if (!remainingNames.remove(outputName)) {
          throw ArgumentError(
            'The generator `$generator` declares an output "$output", which uses '
      ...
      if (remainingNames.isNotEmpty) {
        throw ArgumentError(
          'The generator `$generator` declares an input "$input" using a capture '
  ...
  Iterable<AssetId> matchingOutputsFor(AssetId input) {
    ...
    if (match == null) {
      // The build input doesn't match the input asset, so the generator shouldn't
