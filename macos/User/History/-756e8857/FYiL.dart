...
const scriptPubspecLocation = '$entryPointDir/pubspec.yaml';
...
Method _main() => Method((b) => b
  ...
  ..body = Block.of([
    ...
    refer('sendPort')
        ...
        .call([refer('_generators')]).statement,
    // refer('exitCode', 'dart:io').assign(refer('result')).statement,
    refer('exitCode', 'dart:io').call([literalNum(0)]).statement
  ]));
...
Expression _findToExpression(GeneratorDefinition definition) {
  switch (definition.autoApply) {
    case AutoApply.none:
      return refer(
              'toNoneByDefault', 'package:bonemeal_runner/bonemeal_runner.dart')
          .call([]);
    case AutoApply.dependents:
      return refer(
              'toDependentsOf', 'package:bonemeal_runner/bonemeal_runner.dart')
          .call([literalString(definition.package, raw: true)]);
    case AutoApply.allPackages:
      return refer(
              'toAllPackages', 'package:bonemeal_runner/bonemeal_runner.dart')
          .call([]);
    case AutoApply.rootPackage:
      return refer('toRoot', 'package:bonemeal_runner/bonemeal_runner.dart')
          .call([]);
  }
}
...
Expression _constructBuilderOptions(Map<String, dynamic> options) =>
    refer('GeneratorOptions', 'package:bonemeal/bonemeal.dart')
