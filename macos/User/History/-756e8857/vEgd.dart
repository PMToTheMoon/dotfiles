...
Future<BuildScriptInfo> findBuildScriptOptions({
  ...
  final postProcessGeneratorDefinitions = orderedConfigs
      .expand((c) => c.postProcessGeneratorDefinitions.values)
      .where(_isValidDefinition);

  final applications = [
    for (var builder in orderedBuilders) _applyBuilder(builder),
    for (var builder in postProcessGeneratorDefinitions)
  ...
  final canRunWithSoundNullSafety = await _allMigratedToNullSafety(
      ...
      orderedBuilders
          .map((e) => e.import)
          .followedBy(postProcessGeneratorDefinitions.map((e) => e.import)));
...
/// PostProcessBuilder.
Expression _applyPostProcessBuilder(PostProcessGeneratorDefinition definition) {
  final namedArgs = {
    if (!identical(definition.defaults.generateFor, InputSet.anything))
      'defaultGenerateFor':
          refer('InputSet', 'package:build_config/build_config.dart')
