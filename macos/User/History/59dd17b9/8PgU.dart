  ...
  void _extractCode(String source, Declaration node) {
    switch (extractor) {
      case CodeExtractorType.body:
        _code = extractFunctionOrMethodBodyString(source, node).body;
        break;
  ...
  if (body is BlockFunctionBody) {
    ...
    if (block.childEntities.length > 2) {
      ...
      return ExtractedBody(
        isLambda: false,
        body: source.substring(firstToken.offset, lastToken.end),
      );
    }
  } else if (body is ExpressionFunctionBody) {
    return ExtractedBody(isLambda: true, body: body.expression.toString());
  }
