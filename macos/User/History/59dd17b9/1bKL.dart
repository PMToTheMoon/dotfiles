  ...
  void _extractCode(String source, Declaration node) {
    switch (extractor) {
      case CodeExtractorType.body:
        _code = extractFunctionOrMethodBodyString(source, node).code;
        break;
...
ExtractedBody extractFunctionOrMethodBodyString(
  String source,
...
ExtractedBody _extractFunctionBodyString(String source, FunctionBody body) {
  if (body is BlockFunctionBody) {
    ...
    if (block.childEntities.length > 2) {
      ...
      return ExtractedBody(
        isLambda: false,
        code: source.substring(firstToken.offset, lastToken.end),
      );
    }
  } else if (body is ExpressionFunctionBody) {
    return ExtractedBody(isLambda: true, code: body.expression.toString());
  }
