...
class BuildRunner {
  ...
  final BuildStepReader buildReader;

  final BuildStepWriter buildWriter;

  final BuildCacheReader buildCacheReader;

  Future<BuildResult> run<Input, Output>(
    MetaObject<Input, Output> object,
    Input input,
  ) async {
    // var result = await _runGenerators(phases);
    var result = BuildResult.success();
    final buildStep = BuildStep<Input, Output>(
      input: input,
    );
    object.build(buildStep);
  ...
  const BuildRunner._({
    ...
    required this.buildReader,
    required this.buildWriter,
