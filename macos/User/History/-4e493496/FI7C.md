...
`define`'s method is based on a builder pattern. It's takes a builder function as parameter that have access to a `StateDefinitionBuilder` object and should return it. This object exposes necessary methods to register event handlers and side effects for the defined state.
