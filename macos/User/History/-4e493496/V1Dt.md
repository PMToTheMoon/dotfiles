...
A state can have any number of child states as long their are only defined once. Nested states have access to a `StateDefinitionBuilder` like normal states so they can register they own event handlers, side effects, or in turn, nested states.
...
> 🚨 A state is considered a child as long as it's below the parent state in the state hierarchy. A child state can have it's own children.
