...
`define`'s method is based on a builder pattern. It's takes a builder function as parameter. A `StateDefinitionBuilder` is given as parameter to the builder function. This object exposes necessary methods to register event handlers and side effects for the defined state.
