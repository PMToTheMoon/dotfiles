...
import 'package:bonemeal_core/src/generate/phase.dart';
...
class AssetGraph {
  ...
  static Future<AssetGraph> build(
    ...
    graph
      .._addGeneratorOptionsNodes(buildPhases)
  ...
  /// Adds [GeneratorOptionsAssetNode]s for all [buildPhases] to this graph.
  void _addGeneratorOptionsNodes(List<BuildPhase> buildPhases) {
    for (var phaseNum = 0; phaseNum < buildPhases.length; phaseNum++) {
      var phase = buildPhases[phaseNum];
      if (phase is InBuildPhase) {
        add(GeneratorOptionsAssetNode(
            generatorOptionsIdForAction(phase, phaseNum),
            computeGeneratorOptionsDigest(phase.generatorOptions)));
      } else if (phase is PostBuildPhase) {
        var actionNum = 0;
        for (var builderAction in phase.generatorActions) {
          add(GeneratorOptionsAssetNode(
              generatorOptionsIdForAction(builderAction, actionNum),
              computeGeneratorOptionsDigest(builderAction.generatorOptions)));
          actionNum++;
        }
      } else {
  ...
  Set<AssetId> _removeRecursive(AssetId id, {Set<AssetId>? removedIds}) {
    ...
    if (node is NodeWithInputs) {
      ...
      if (node is GeneratedAssetNode) {
        get(node.generatorOptionsId)!.outputs.remove(id);
      }
    }
  ...
  bool _actionMatches(BuildAction action, AssetId input) {
    ...
    if (action is InBuildPhase) {
      if (!action.generator.hasOutputFor(input)) return false;
    } else if (action is PostBuildAction) {
      var inputExtensions = action.generator.inputExtensions;
  ...
  Set<AssetId> _addInBuildPhaseOutputs(
    ...
    var buildOptionsNodeId = generatorOptionsIdForAction(phase, phaseNum);
    var generatorOptionsNode =
        get(buildOptionsNodeId) as GeneratorOptionsAssetNode;
    var inputs =
    ...
    for (var input in inputs) {
      ...
      var node = get(input)!;
      var outputs = expectedOutputs(phase.generator, input);
      phaseOutputs.addAll(outputs);
      node.primaryOutputs.addAll(outputs);
      var deleted = _addGeneratedOutputs(
          outputs, phaseNum, generatorOptionsNode, buildPhases, rootPackage,
  ...
  void _addPostBuildPhaseAnchors(PostBuildPhase phase, Set<AssetId> allInputs) {
    var actionNum = 0;
    for (var action in phase.generatorActions) {
      var inputs = allInputs.where((input) => _actionMatches(action, input));
      for (var input in inputs) {
        var buildOptionsNodeId = generatorOptionsIdForAction(action, actionNum);
  ...
  Set<AssetId> _addGeneratedOutputs(
      ...
      int phaseNumber,
      GeneratorOptionsAssetNode generatorOptionsNode,
      List<BuildPhase> buildPhases,
      String rootPackage,
    ...
    for (var output in outputs) {
      ...
      var newNode = GeneratedAssetNode(output,
          ...
          wasOutput: false,
          isFailure: false,
          generatorOptionsId: generatorOptionsNode.id,
          isHidden: isHidden);
      if (existing != null) {
      ...
      generatorOptionsNode.outputs.add(output);
      _add(newNode);
    }
    return removed;
  }
...
Digest computeGeneratorOptionsDigest(GeneratorOptions options) =>
    md5.convert(utf8.encode(json.encode(options.config)));

AssetId generatorOptionsIdForAction(BuildAction action, int actionNum) {
  if (action is InBuildPhase) {
    return AssetId(action.package, 'Phase$actionNum.generatorOptions');
  } else if (action is PostBuildAction) {
    return AssetId(action.package, 'PostPhase$actionNum.generatorOptions');
  } else {
