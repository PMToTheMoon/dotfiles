...
class PostBuildPhase extends BuildPhase {
  final List<PostBuildAction> generatorActions;
  ...
  PostBuildPhase(this.generatorActions);

  @override
  String toString() => generatorActions.map((a) => a.generatorLabel).join(', ');

  @override
  int get identity =>
      _deepEquals.hash(generatorActions.map<dynamic>((b) => b.identity).toList()
        ..addAll([
          isOptional,
          hideOutput,
        ]));
...
/// [PostProcessGenerator] to a single [package] with some additional options.
class PostBuildAction implements BuildAction {
  @override
  final PostProcessGenerator generator;
  @override
  final String generatorLabel;
  @override
  final GeneratorOptions generatorOptions;
  ...
  PostBuildAction(this.generator, this.package,
      {String? generatorKey,
  ...
  int get identity => _deepEquals.hash([
        generatorLabel,
        generator.inputExtensions.toList(),
        generateFor,
        package,
        targetSources,
      ]);
