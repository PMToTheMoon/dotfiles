...
class StreamViewBloc extends YamlMetaObject<StreamViewBlocData> {
  ...
  FutureOr<void> build(BuildContext context, StreamViewBlocData data) async {
    ...
    final iprBlocFileContent = renderDartTemplate(
      blocTemplate,
      context.source.uri,
      await data.toParams(context, modelMetaData, isIpr: true),
    );
    final outputBlocFileContent = renderDartTemplate(
      blocTemplate,
      context.source.uri,
      await data.toParams(context, modelMetaData, isIpr: false),
    );

    await context.ipr('bloc/${data.fileName}', iprBlocFileContent);
