...
class BonemealConfig {
  ...
  BonemealConfig({
    ...
    GlobalBonemealConfig? globalConfig,
        ...
        globalConfig = globalConfig ?? GlobalBonemealConfig.empty,
...
class GlobalBonemealConfig {
  static const empty = GlobalBonemealConfig();

  final Map<String, GeneratorConfig> generators;

  /// Create [GlobalBonemealConfig] with sain default
  GlobalBonemealConfig({
    Map<String, GeneratorConfig>? generators,
  }) : generators = generators ?? const {} {
    generators = this.generators.map(
          (key, value) => MapEntry(normalizeGeneratorKeyUsage(key), value),
        );
  }
