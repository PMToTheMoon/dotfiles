...
class _HomeRecords extends StatelessWidget {
  ...
  void _onDeleteRecord(
    ...
    context.showConfirmationDialog('Supprimer le brouillon ?', message, onAccept: onAccept)
            ...
            child: BlocBuilder<HomeRecordsBloc, HomeRecordsState?>(
              builder: (context, HomeRecordsState? state) => TabBarView(
                children: [
                  ...
                  HomeRecordsList(
                    data: state?.drafts.map((record) {
                      final b = record.doctorId == userId;
                      return record.toRecordCardWithEditButtonData(
                          userId: userId!,
                          onTap: () => _onSelectRecord(context, record),
