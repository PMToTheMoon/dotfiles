...
final Step<void> writeStep = Step.define(
  ...
  for (final ctxAsset in contextAssets) {
    // await fs.file(ctxAsset.path).writeAsString(ctxAsset.content);
    await Directory(ctxAsset.path).create(recursive: true);
    await File(ctxAsset.path).writeAsString(ctxAsset.content);
  }
});
