...
enum AssetType {
  ...
  int compareTo(AssetType other) => value - other.value;
}
...
class AssetId implements Comparable<AssetId> {
  ...
  AssetId.input(this.path) : this.type = AssetType.input;
  AssetId.ipr(this.path) : this.type = AssetType.ipr;
  AssetId.output(this.path) : this.type = AssetType.output;
  ...
  factory AssetId.resolve(Uri uri, {AssetId? from}) {
    ...
    throw UnsupportedError(
        'Cannot resolve $uri; only "input", "ipr" and "output" schemes supported');
  }
...
Uri _constructUri(AssetId id) {
  ...
  return Uri(scheme: scheme, pathSegments: [id.type.name, ...pathSegments]);
}
