      ...
      children: [
        ...
        if (state.skills.isNotEmpty)
          Wrap(
            ...
            runSpacing: Dimensions.paddingSmaller,
            children: state.skills
                .map((skill) => _SkillChip(
                    skill.name, state.pendingDeletions.contains(skill)))
                .toList(),
          ),
      ],
    );
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: context.bodySmall(
        context.l10n.candidate_skills_list_empty_text,
  ...
  Widget build(BuildContext context) {
    ...
    return InputChip(
      ...
      onDeleted: () => context.read<CandidateSkillListBloc>()
        ..add(CandidateSkillListEvent.skillDeleted(skill: skill.name)),
    );
  }
}
