...
class TripleBlocListener extends MetaObject {
  TripleBlocConsumer(this.bloc);
...
Parameters _listenerParametersFromBloc(TripleStateBlocMeta bloc) {
  return Parameters({
    for (final state in _allStatesExceptInitial(bloc.states.types.values))
      namedParam('on${state.name.pascalCase}', required: false),
  });
}
...
Parameter _initialBuilderParameter(TripleStateBlocMeta bloc) {
  return namedParam('initial', required: false);
}

Field _initialBuilderField(TripleStateBlocMeta bloc) {
  return field(
    'initial',
    refer(
      'Widget Function(BuildContext context, ${bloc.states['initial'].$class.name})?',
    ),
  );
}
