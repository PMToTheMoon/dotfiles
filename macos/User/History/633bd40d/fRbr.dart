...
class TbtRepository extends Seed {
  ...
  Iterable<String> get _clients => items.map((e) => e.client).toSet();

  Iterable<Code> get serviceInitializers => clients.map(
      (client) => Code('_$client = authRepository.tbtPrivateClient.$client'));

  Iterable<Field> get serviceFields => clients
      .map((client) => field('_$client', refer('${client.pascalCase}Service')));
}
