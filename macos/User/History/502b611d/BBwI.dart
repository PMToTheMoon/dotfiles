...
class CandidateRepository {
  ...
  Future<void> createEducation(
    NewEducation experience,
  ) async {
    await Future.delayed(const Duration(seconds: 1));
    final List<Education> experiences = _educations ?? [];
    experiences.add(
      Education(
    ...
    _educations = experiences;
    _educationsStream.add(experiences);
  }
  ...
  Future<void> updateEducation(
    ...
    if (_educations != null) {
      final experienceIndex =
          _educations!.indexWhere((element) => element.id == update.id);
      final e = _educations![experienceIndex];
      _educations?[experienceIndex] = Education(
        id: e.id,
        title: update.title ?? e.title,
        company: update.company ?? e.company,
        location: update.location ?? e.location,
