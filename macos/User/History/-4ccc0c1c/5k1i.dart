...
class ChipSelector extends StatefulWidget {
  const ChipSelector({
    ...
    super.key,
        ...
        assert(maxChipSelected == null || maxChipSelected > 0);
...
class _ChipSelectorState extends State<ChipSelector> {
  ...
  void _onChipTapped(String chip, bool selected) {
    if (selected) {
      final limit = widget.maxChipSelected;
      if (limit != null && _controller.value.length >= limit) {
        _controller.unselect(_controller.value.first);
      }
