...
Future<String> _generateBuildScript() async {
  ...
  final library = Library((b) => b.body.addAll([
        literalList(
                info.metaObjectApplications,
                refer('MetaObjectApplication',
                    'package:bonemeal_runner/bonemeal_runner.dart'))
            .assignFinal('_metaObjects')
            .statement,
        literalList(
...
Method _main() => Method((b) => b
  ...
  ..body = Block.of([
    refer('run', 'package:bonemeal_runner/bonemeal_runner.dart')
        .call([refer('args'), refer('_metaObjects'), refer('_generators')])
        .awaited
        .assignVar('result')
        .statement,
    refer('sendPort')
...
Expression _applyMetaObject(MetaObjectDefinition definition) {
  final namedArgs = {
    if (definition.defaults.options.isNotEmpty)
      'defaultOptions':
          _constructMetaObjectOptions(definition.defaults.options),
    if (definition.defaults.devOptions.isNotEmpty)
      'defaultDevOptions':
          _constructMetaObjectOptions(definition.defaults.devOptions),
    if (definition.defaults.releaseOptions.isNotEmpty)
      'defaultReleaseOptions':
          _constructMetaObjectOptions(definition.defaults.releaseOptions),
  };
  ...
  return refer(
    ...
    refer(definition.factory, import),
  ], namedArgs);
}
...
Expression _applyBuilder(GeneratorDefinition definition) {
  ...
  return refer('apply', 'package:bonemeal_runner/bonemeal_runner.dart').call([
    literalString(definition.key, raw: true),
    literalList([for (var f in definition.factories) refer(f, import)]),
  ], namedArgs);
}
...
Expression _constructMetaObjectOptions(Map<String, dynamic> options) =>
    refer('MetaObjectOptions', 'package:bonemeal_core/bonemeal_core.dart')
