...
class PaymentMethodsBloc
  ...
  Future<void> _onNewPaymentMethodAdded(
    NewPaymentMethodAdded event,
    Emitter<PaymentMethodsState> emit,
  ) async {
    await userRepository.fetch();
    final lastMethod = userRepository.paymentMethods?.last;
    if (lastMethod != null) {
      if (lastMethod is CostCenterPamentMethod)
    }
  }
}
