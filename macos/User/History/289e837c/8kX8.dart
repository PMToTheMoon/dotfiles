...
class TripleStateBloc extends MetaObject<TripleStateBlocMeta> {
  factory TripleStateBloc({
    ...
    final parameters = source.fetch.allParamsAsProps
    ...
    return TripleStateBloc._(
      ...
      source: source,
      data: data,
      sourceImmutableParameters: Properties(parameters),
  ...
  TripleStateBloc._({
    ...
    required this.data,
    this.sourceImmutableParameters,
  ...
  Properties? sourceImmutableParameters;

  Properties get blocParameters => Properties({
        source.toProp(),
  ...
  Iterable<Method> methods(FreezedMetaData events, FreezedMetaData states) {
    final fetch = _FetchMethod(
      dataName: data?.name,
      sourceFetchValueResolver: _DefaultSourceValueResolver(blocParameters),
      source: source,
  ...
  TripleStateBlocMeta build(BuildStep step) {
    final blocMeta = step.build(Bloc(
      subject: subject,
      properties: blocParameters,
      states: {
        'initial': {},
        'loading': {
      ...
      events: {
        fetchEvent: Properties(
          source.fetch.allParamsAsProps
              .difference(sourceImmutableParameters ?? {}),
        ).toFields(),
        // 'updateRequested': {},
      },
      statesHooks: [
...
class _DefaultSourceValueResolver extends ValueResolver {
  ...
  final Properties blocParameters;
