...
class TripleStateBloc extends Bloc {
  factory TripleStateBloc({
    ...
    final data = field(dataName ?? 'data', source.fetch.returns!);
    final parameters = source.fetch.allParameters
        .where((p) => sourceImmutableParameters.contains(p.name));
    return TripleStateBloc._(
      subject: subject,
      source: source,
      data: data,
      parameters: sourceImmutableParameters,
    );
  }
...
class _FetchMethod extends Hook {
  ...
  Method get method {
    ...
    final handler = eventHandler(events['started'])
      ..body = Block.of(body)
