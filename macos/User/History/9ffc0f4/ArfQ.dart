...
class HomeRecordsBloc extends Cubit<HomeRecordsState?> {
  HomeRecordsBloc({
    ...
    required HomeRecordsState initialRecords,
    required Stream<List<CspUserRecord>?> recordsStream,
  }) : super(initialRecords) {
    _streamSubscription = recordsStream.listen((data) => emit(
          _filterRecords(data),
        ));
    _streamSubscription = recordsStream.map(_filterRecords).listen(emit);
  }
