...
A good practice is to keep all constants inside a dedicated class so all your widget can access it and share same values:
```dart
class Dimensions {
  // Blank Spaces
...
Neat provide a set of generator that can parse such resource classes and generate useful widget with pre-filled values.
