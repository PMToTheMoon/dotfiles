...
```dart
PaddingSmall(),
PaddingSmall.horizontal(),
PaddingSmall.vertical(),
PaddingSmall(right), // only available if generateBinaryFlagConstructor is enabled
PaddingSmall(top | right), // only available if generateBinaryFlagConstructor is enabled
```
