...
class PatientRecordsView extends StatefulWidget {
  ...
  final void Function(PatientRecordView)? onRecordSelected;
...
class _PatientRecordsView extends StatelessWidget {
  ...
  Widget _recordBuilder(BuildContext context, PatientRecordView record) {
    return RecordTile(
      RecordCardData.patientRecord(
        recordTitle: (record.lastModified ?? record.dateCreated)
            .toString(), //record.recordType.displayName(context),
        image: avatarCacheService.recordImage(record.recordType),
