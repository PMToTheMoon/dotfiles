...
class PhoneMenuDisplay extends StatelessWidget {
  PhoneMenuDisplay({
    ...
    Key? key,
  })  : _formatedNumber = _formatPhoneNumber(number),
        super(key: key);

  final IconData icon;

  final Future<String> _formatedNumber;
...
Future<String> _formatPhoneNumber(String number) async {
  final nmbr = await PhoneNumberUtil().parse(number);
  return PhoneNumberUtil().format(number, nmbr.countryCode);
}
