...
void main() {
  test('build.yaml can be parsed', () {
    var buildConfig = BonemealConfig.parse('example', ['a', 'b'], buildYaml);
    ...
    expectBuilderDefinitions(buildConfig.generatorDefinitions, {
      'example:h': createBuilderDefinition(
        ...
        defaults: TargetGeneratorConfigDefaults(
    ...
    expectGlobalOptions(buildConfig.globalOptions, {
      'example:h': GlobalBonemealConfig(options: const {'foo': 'global'}),
      'b:b': GlobalBonemealConfig(
  ...
  test('build.yaml can omit a targets section', () {
    var buildConfig =
        BonemealConfig.parse('example', ['a', 'b'], buildYamlNoTargets);
    expectBuildTargets(buildConfig.buildTargets, {
      'example:example': createBuildTarget(
        'example',
  ...
  test('build.yaml can be empty', () {
    var buildConfig = BonemealConfig.parse('example', ['a', 'b'], '');
    expectBuildTargets(buildConfig.buildTargets, {
      'example:example': createBuildTarget(
        'example',
        dependencies: {'a:a', 'b:b'},
...
''');
    expect(buildConfig.generatorDefinitions.keys, ['example:example']);
    expect(buildConfig.generatorDefinitions['example:example']!.runsBefore,
        ['a:foo_builder']);
    expect(buildConfig.generatorDefinitions['example:example']!.appliesBuilders,
        ['a:foo_builder']);
  });
}
...
void expectBuilderDefinitions(Map<String, GeneratorDefinition> actual,
    Map<String, GeneratorDefinition> expected) {
...
void expectGlobalOptions(Map<String, GlobalBonemealConfig> actual,
    Map<String, GlobalBonemealConfig> expected) {
  expect(actual.keys, unorderedEquals(expected.keys));
  for (var p in actual.keys) {
...
Matcher _matchesBuilderDefinition(GeneratorDefinition definition) =>
    TypeMatcher<GeneratorDefinition>()
        .having((d) => d.generatorFactories, 'builderFactories',
            definition.generatorFactories)
...
Matcher _matchesGlobalBuilderConfig(GlobalBonemealConfig config) =>
    TypeMatcher<GlobalBonemealConfig>()
...
Matcher _matchesBuilderConfigDefaults(TargetGeneratorConfigDefaults defaults) =>
    TypeMatcher<TargetGeneratorConfigDefaults>()
...
BuildTarget createBuildTarget(String package,
  ...
  return runInBuildConfigZone(() {
    ...
    generatorKeyExpando[target] = key ?? '$package:$package';

    return target;
  }, package, []);
}

GeneratorDefinition createBuilderDefinition(String package,
    {required List<String> builderFactories,
    AutoApply? autoApply,
    bool? isOptional,
    BuildTo? buildTo,
    required String import,
    required List<String> buildExtensions,
    String? key,
    Iterable<String>? requiredInputs,
    Iterable<String>? runsBefore,
    Iterable<String>? appliesBuilders,
    TargetGeneratorConfigDefaults? defaults}) {
  return runInBuildConfigZone(() {
    var definition = GeneratorDefinition(
        generatorFactories: builderFactories,
        autoApply: autoApply,
    ...
    generatorKeyExpando[definition] = key ?? '$package:$package';
    return definition;
  }, package, []);
}
