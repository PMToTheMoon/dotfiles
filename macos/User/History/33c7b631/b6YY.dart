...
class _SwingState extends State<AlbumSwing>
  ...
  void initState() {
    ...
    animationRotation3 = Tween<double>(begin: widget._minRotation, end: 0)
        .animate(CurvedAnimation(
            parent: controller!,
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedBuilder(
        ...
        builder: (BuildContext context, Widget? child) {
          double angle = (animationRotation1.value != widget._maxRotation)
              ? animationRotation1.value
              : (animationRotation2.value != widget._minRotation)
...
class Suspended extends StatefulWidget {
  const Suspended({
    ...
    this.maxXShift = 0.5,
    this.holeSize = 8,
    this.minSwingForce = 0.2,
    this.maxSwingForce = 0.5,
    Key? key,
...
class _SuspendedState extends State<Suspended> {
  ...
  void initState() {
    ...
    _rotation =
        widget.minAngle + (_randomValue * (widget.maxAngle - widget.minAngle));
    _ropeXAlign =
        -widget.maxXShift + ((1 - _randomValue) * (widget.maxXShift * 2));
  ...
  int get _animDuration =>
      ((_maxAnimDurationMS - _minAnimDurationMS) * widget.random.nextDouble())
          .toInt() +
      _minAnimDurationMS;

  double get rotationForce =>
      widget.minSwingForce +
      ((widget.maxSwingForce - widget.minSwingForce) *
