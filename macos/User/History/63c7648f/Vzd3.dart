...
class CandidateProfileBloc
  ...
  final CandidateCandidateRepository candidateRepository;

  late final StreamSubscription<Candidate> _candidateStreamSubscription;

  void _onStarted(CandidateProfileStarted event, Emitter emit) {
    final candidate = candidateRepository.candidate;
    if (candidate != null) emit(CandidateProfileReady(candidate: candidate));
    try {
      candidateRepository.fetchCurrentCandidate();
    }
  }
