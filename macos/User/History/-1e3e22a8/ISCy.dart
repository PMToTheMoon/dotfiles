...
class HubBloc extends Bloc<HubEvent, HubState> {
  ...
  Future<void> _onFetchRequested(HubFetchRequested event, Emitter emit) async {
    ...
    try {
      ...
      final effectiveRecord = _findEffectiveRecord(userId, rootRecord);

      emit(
        HubReady(
          rootRecord: rootRecord,
          effectiveRecord: effectiveRecord,
          showInitialObservationForm: effectiveRecord.status.isDraft,
      ...
      // Tmp code to show when record is forbidden
      // Not clear what to do about it
      if (e is DioError && e.response?.statusCode == 403) {
        emit(const HubError('Accès refusé'));
      } else {
        emit(const HubError());
      }
    }
  ...
  Record _findEffectiveRecord(String userId, Record rootRecord) {
    if (userId == rootRecord.doctorId || userId == rootRecord.providerId) {
      return rootRecord;
    }
