...
import 'package:bonemeal_runner/src/generate/build_directory.dart';
...
Future<bool> createMergedOutputDirectories(
    BuildDirectory iprBuildDir,
    Set<BuildDirectory> outputsDirs,
  ...
  var conflictingOutputs = _conflicts({iprBuildDir, ...outputsDirs});
...
Future<bool> _createMergedOutputDir(
  ...
  try {
    ...
    await logTimedAsync(_logger, 'Creating merged output dir `$outputPath`',
      ...
      outputAssets.addAll(await Future.wait([
        for (var id in builtAssets)
          _writeAsset(id, iprDir, outputDir, root, packageGraph, reader,
              symlinkOnly, hoist),
        // _writeCustomPackagesFile(packageGraph, iprDir, outputDir),
        // if (await reader.canRead(_packageConfigId(packageGraph.root.name)))
        //   _writeModifiedPackageConfig(
        //       packageGraph.root.name, reader, iprDir, outputDir),
      ]));
    ...
    //       iprDir, outputDir, AssetId.input(_manifestName), content);
    // });
...
Future<AssetId> _writeCustomPackagesFile(
    PackageGraph packageGraph, Directory iprDir, Directory outputDir) async {
  var packagesFileContent =
      packageGraph.allPackages.keys.map((p) => '$p:packages/$p/').join('\r\n');
  var packagesAsset = AssetId.input('.packages');
  await _writeAsString(iprDir, outputDir, packagesAsset, packagesFileContent);
  return packagesAsset;
}

AssetId _packageConfigId(String rootPackage) =>
    AssetId.input('.dart_tool/package_config.json');
...
Future<AssetId> _writeModifiedPackageConfig(String rootPackage,
    AssetReader reader, Directory iprDir, Directory outputDir) async {
  ...
  await _writeAsString(
      iprDir, outputDir, packageConfigAsset, jsonEncode(packageConfig));
...
Future<AssetId> _writeAsset(
  ...
  return _descriptorPool.withResource(() async {
    String assetPath;
    if (id.type == AssetType.output) {
      assetPath = p.url.join(outputDir.path, id.path);
    } else if (id.type == AssetType.ipr) {
      final assetPathTest = p.url.join(outputDir.path, id.path);
      assetPath = id.path;
      if (hoist && p.isWithin(root, id.path)) {
        assetPath = p.relative(id.path, from: root);
      }
    } else {
      throw 'cannot write an input asset';
    }

    var outputId = AssetId(id.type, assetPath);
    try {
      if (symlinkOnly) {
        await Link(_filePathFor(iprDir, outputDir, outputId)).create(
        ...
        await _writeAsBytes(
            iprDir, outputDir, outputId, await reader.readAsBytes(id));
      }
...
Future<void> _writeAsBytes(
        Directory iprDir, Directory outputDir, AssetId id, List<int> bytes) =>
    _fileFor(iprDir, outputDir, id).then((file) => file.writeAsBytes(bytes));

Future<void> _writeAsString(
        Directory iprDir, Directory outputDir, AssetId id, String contents) =>
    _fileFor(iprDir, outputDir, id)
        .then((file) => file.writeAsString(contents));

Future<File> _fileFor(Directory iprDir, Directory outputDir, AssetId id) {
  return File(_filePathFor(iprDir, outputDir, id)).create(recursive: true);
}

String _filePathFor(Directory iprDir, Directory outputDir, AssetId id) {
  if (id.type == AssetType.output) {
    return p.join(outputDir.path, id.path);
  } else if (id.type == AssetType.ipr) {
    return p.join(iprDir.path, id.path);
  } else {
    throw 'cant write input asset ${id.path}';
  }
}
