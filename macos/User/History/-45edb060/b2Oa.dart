...
class {{ event_class }} with _${{ event_class }} {
  const factory {{ event.started.ctr }} = {{ states.started.class }};

  const factory {{ event.fetch_succeed.ctr }}({
    required {{ source.data.type }} {{ source.data.name }},
  }) = {{ event.fetch_succeed.class }};

  const factory {{ event.fetch_failed.ctr }} = {{ event.ready.class }};
