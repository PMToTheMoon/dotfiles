...
class {{ bloc_subject }}State with _${{ bloc_subject }}State {
  const factory {{ state.initial.ctr }} = {{ state.initial.class }};

  const factory {{ state.fetching.ctr }} = {{ state.fetching.class }};

  const factory {{ state.ready.ctr }}({
    required {{ source.data.type }} {{ source.data.name }},
  }) = {{ state.ready.class }};

  const factory {{ state.error.ctr }} = {{ state.error.class }};
}
''';
