...
class IOEnvironment implements BuildEnvironment {
  @override
  final PathProvidingAssetReader reader;
  ...
  Future<BuildResult> finalizeBuild(
    ...
    if (buildResult.status == BuildStatus.success) {
      try {
        try {
          await _tryExportDir(iprBuildCacheDir, iprDir);
          _logger.fine('Ipr build cache successfully exported.');
        } on UnableToExportException {
          _logger.info('No build asset produced for ipr.');
        }

        if (outputDir != null) {
          try {
            await _tryExportDir(outputBuildCacheDir, outputDir);
            _logger.fine('Output build cache successfully exported.');
          } on UnableToExportException {
            _logger.info('No build asset produced for output.');
          }
        } else {
          _logger
              .info('No output export dir specified. Skipping output export.');
        }
      } catch (e) {
...
class UnableToExportException implements IOException {
  const UnableToExportException(this.reason);
  final String reason;
}

Future<void> _tryExportDir(String from, String to) async {
  final buildCacheDir = Directory(from);
  final exportDir = Directory(to);
  if (await buildCacheDir.exists() == true) {
    // TODO: clean export dir before build
    if (await exportDir.exists() == false) {
      await exportDir.create(recursive: true);
    }
    await copyPath(from, to);
  } else {
    throw const UnableToExportException(('Source directory does not exists.'));
  }
}
