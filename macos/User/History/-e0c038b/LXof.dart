...
class UrbanDriverApiAuthClient {
  ...
  Future<domain.FlightData> flightData({
    ...
    try {
      ...
      final result = FlightStatus.fromJson(response.data!);
      return result.toDomain(flightNumber);
    } catch (e) {
      throw _handleError(e);
    }
  }
  ...
  Future<String> sessions() async {
    try {
      final response = await _client.get<Map<String, dynamic>>(
        "/sessions",
      );
      final url = response.data?["ok"];
      if (url == null || url is! String) throw "Request failed";
      return url;
    } catch (e) {
      throw _handleError(e);
    }
