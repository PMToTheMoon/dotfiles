...
class RepositoryMetaData {
  ...
  T source<T>(String name) {
    assert(() {
      final s = sources[name];
      if (s == null) {
        throw '''
Trying to access a source that didn't exist:
$name form ${reference.symbol} repository
available sources: ${sources.keys}''';
      } else if (s is! T) {
        throw '''
The source $name isn't of type $T.
source type: ${s.runtimeType}''';
      }
      return true;
    }());
...
class Repository extends MetaObject<RepositoryMetaData> {
  ...
  Map<String, RepositorySource> _buildSourcesMeta(Reference repository) {
    ...
    for (final item in items) {
      ...
      sources.add(MapEntry(item.name, meta));
    }
