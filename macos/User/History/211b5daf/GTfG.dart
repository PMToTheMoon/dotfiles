...
class Repository extends MetaObject<RepositoryMetaData> {
  ...
  RepositoryMetaData build(BuildStep step) {
    ...
    final sourcesMeta = _buildSourcesMeta();
  ...
  Map<String, RepositorySource> _buildSourcesMeta {
    final meta = <MapEntry<String, RepositorySource>>[];
    for (final item in items) {
sources.add(      MapEntry(item.name, item.meta(repositoryReference),),);
    }
    return Map.fromEntries(meta);
  }
}
