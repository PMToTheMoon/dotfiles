...
class _AddressSelectionPageBodyState extends State<_AddressSelectionPageBody> {
  ...
  Future<void> addressSelected(Address address, bool isDeparture) async {
    late final Address correctDepartureAddress;
    if (isDeparture) {
      correctDepartureAddress = await _onDepartureSelected(address);
    } else {
      await _onArrivalSelected(address);
    }
    final bloc = context.read<AddressSelectionBloc>();
    switch (bloc.state.currentFocus) {
      case AddressSelectionFocus.departure:
        _controller.departure = correctDepartureAddress.label;
        focusArrival();
        _controller.focusArrival();
        break;
      case AddressSelectionFocus.arrival:
        _controller.arrival = address.label;
        focusDeparture();
        _controller.focusDeparture();
        break;
    }
  ...
  Future<Address> _onDepartureSelected(Address address) async {
    DateTime? date;
    String? trainNumber;
    String? flightNumber;
    int? departureDelay;
    Address _selectedAddress = address;
    if (address.type == AddressType.train) {
      final result = await TrainNumberSelectionPage.open(
        context,
        const TrainNumberSelectionPageArguments(),
      );
      trainNumber = result.trainNumber;
    } else if (address.type == AddressType.airport) {
      final state = context.read<AddressSelectionBloc>().state;
    ...
    final bloc = context.read<AddressSelectionBloc>();
    bloc.add(AddressSelectionEvent.departureAddressSelected(
  ...
  Future<void> _onArrivalSelected(Address address) async {
    final bloc = context.read<AddressSelectionBloc>();
    bloc.add(AddressSelectionEvent.arrivalAddressSelected(
      address: address,
    ));
  }
