...
import 'package:zanalys/features/upload_document/bloc/upload_document_event.dart';
import 'package:zanalys/features/upload_document/bloc/upload_document_state.dart';
import 'package:zanalys/features/upload_document/validators/access_code.dart';
import 'package:zanalys/features/upload_document/validators/document.dart';
import 'package:zanalys/features/upload_document/validators/document_type_validator.dart';
...
class UploadDocumentBloc
  ...
  void _onDocumentAccessCodeUpdated(DocumentAccessCodedUpdated event,
    ...
    emit(state.copyWith(
      ...
      status: Formz.validate([state.document, code]),
    ));
  }
  ...
  void _onDocumentSelected(
    ...
    if (state.status.isValidated) {
      ...
      try {
        final result = await AppServices.network.uploadDocument(
          path: document.value,
        ...
        if (e is DioError && e.response?.statusCode == 401) {
          //Do nothing, handled in Dio client
        } else {
