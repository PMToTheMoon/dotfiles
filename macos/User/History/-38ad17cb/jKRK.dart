...
class SimpleFetchBloc extends Seed {
  factory SimpleFetchBloc({
    ...
    return SimpleFetchBloc._(
      ...
      dataName: dataName,
      transformer: transformer,
      initialStateValues: initialStateValues ?? const DefaultValueResolver(),
  ...
  Map<String, Set<Field>> get _events {
    final sourceHandlers = [];

    for (final param in source.method.allParameters) {
      if (fields.any((f) => f.name == param.name)) continue;
      sourceHandlers.add(
        MapEntry(
  ...
  Map<String, Set<Field>> get _states {
    final fieldsFromSource = source.method.toFields().exclude(fields);
    return {
      'initial': source.method.toFields(),
      'loading': {
  ...
  List<Method> _methods(
    ...
    final fetch = _FetchMethod(events: events, states: states);

    return [
      fetch.method,
      ..._fetchParametersHandlers(events),
  ...
  Iterable<Method> _fetchParametersHandlers(FreezedMetaData events) {
...
class _FetchMethod extends Hook {
  ...
  Code _callTransformer() {
    return transformer!.entrypoint.call().assignFinal('success').statement;
  }
