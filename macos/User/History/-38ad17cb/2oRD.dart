...
class Source {
  Source.fromRepositoryFutureSource(RepositoryFetchableSource source)
...
class ParametrizedFetchBloc extends Seed<void> {
  factory ParametrizedFetchBloc({
    ...
    assert(() {
      if (source.method.returns == refer('void')) {
        throw '''
Sources that returns void are forbidden.
  -> FetchBloc $subject source : ${source.method.name}''';
      }
      return true;
    }());
    final blocParameters = Parameters(parameters);
    ...
    return ParametrizedFetchBloc._(
      subject: subject,
      source: source,
      dataType: dataType,
      dataName: dataName,
      ...
      sourceValues: sourceValues ?? _DefaultSourceValueResolver(blocParameters),
      parameters: blocParameters,
    );
  }

  ParametrizedFetchBloc._({
  ...
  @override
  MetaObject<void> grow(BuildContext context) {
    return Bloc(
  ...
  Parameters _sourceVariableParameters() => Parameters({
        ...source.method.allParameters.whereNot(
          (p) => parameters.any((f) => f.name == p.name),
        )
      });
}
