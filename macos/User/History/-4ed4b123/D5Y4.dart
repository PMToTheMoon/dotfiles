...
String? _methodType(ExecutableElement element) {
  if (element is PropertyAccessorElement) {
    if (element.isGetter) return 'MethodType.getter';
    if (element.isSetter) return 'MethodType.setter';
  }
  return null;
}
...
Iterable<String> _requiredParameters(ExecutableElement element) {
  return element.parameters
      .where((element) => element.isRequiredPositional == true)
      .map((p) => """
Parameter((p) => p
  ..name = '${p.name}'
...
Iterable<String> _optionalParameters(ExecutableElement element) {
  return element.parameters
      .where((element) => element.isRequiredPositional == false)
      .map((p) => """
Parameter((p) => p
  ..name = '${p.name}'
  ..type = ${_referType(p.type)}
