...
class BuildCacheAssetWriter extends AssetWriter implements RunnerAssetWriter {
  ...
  Future<void> writeAsString(
    ...
    _assetGraph.add(id, generated: true);
    return writeAsString(id, contents);
  }
...
class _RelocatedAssetWriter implements RunnerAssetWriter {
  final AssetGraph _assetGraph;
  final RunnerAssetWriter _delegate;
  final String _rootPackage;
  final String root;

  _RelocatedAssetWriter(this._delegate, this._assetGraph, this._rootPackage);

  @override
  Future writeAsBytes(AssetId id, List<int> content) =>
      _delegate.writeAsBytes(_location(id, _assetGraph, _rootPackage), content);

  @override
  Future writeAsString(AssetId id, String content,
          {Encoding encoding = utf8}) =>
      _delegate.writeAsString(_location(id, _assetGraph, _rootPackage), content,
          encoding: encoding);

  @override
  Future delete(AssetId id) =>
      _delegate.delete(_location(id, _assetGraph, _rootPackage));
}

AssetId _location(AssetId id, String rootPackage, String location) {
  if (id.path.startsWith(location)) {
    return id;
  }
  return AssetId(
    rootPackage,
    '$location/${id.package}/${id.path}',
  );
}
