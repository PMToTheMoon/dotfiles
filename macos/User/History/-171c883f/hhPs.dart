...
class FileBasedAssetReader extends AssetReader
  ...
  Future<bool> canRead(AssetId id) =>
      _descriptorPool.withResource(() => _fileFor(id).exists());

  @override
  Future<List<int>> readAsBytes(AssetId id) => _fileForOrThrow(id)
      .then((file) => _descriptorPool.withResource(file.readAsBytes));

  @override
  Future<String> readAsString(AssetId id, {Encoding encoding = utf8}) =>
      _fileForOrThrow(id).then((file) => _descriptorPool
  ...
  @override
  String pathTo(AssetId id) => id.path;
}
...
AssetId _fileToAssetId(File file, PackageNode packageNode) {
  ...
  return AssetId(AssetType.input, relativePath);
}
...
Future<File> _fileForOrThrow(AssetId id) {
  var file = _fileFor(id);
