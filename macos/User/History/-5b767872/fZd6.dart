...
class _CommandConfirmationDialogState extends State<CommandConfirmationDialog> {
  ...
  String get departureLabel {
    if (widget.reservation.flightNumber != null) {
      return widget.reservation.departure.label +
          " (${widget.reservation.flightNumber})";
    } else if (widget.reservation.trainNumber != null) {
      return widget.reservation.departure.label +
          " (${widget.reservation.trainNumber})";
    }
    return widget.reservation.departure.label;
  }
        ...
        children: [
          _DialogItem(
            ...
            text: DateFormat.yMMMd(context.l10n.localeName)
                .add_jm()
                .format(widget.reservation.date.add(
                  Duration(minutes: widget.reservation.departureDelay ?? 0),
                )),
          ),
          const Space4(),
          _DialogItem(
          ...
          if (!_showCommentField)
            Button.menu(
              context.l10n.recap_popup_add_comment,
              () => setState(() {
                _showCommentField = true;
              }),
              color: context.colorScheme.tertiary,
            ),
          if (_showCommentField) _CommentField(controller: _comment),
          const Space4(),
          Row(
            children: [
              Expanded(
