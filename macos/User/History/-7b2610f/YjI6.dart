...
class BuildStep extends PathProvidingAssetReader implements AssetWriter {
  ...
  AssetId _ipr(String path) => AssetId.ipr(_contextualizedPath(_root, path));

  AssetId _output(String path) =>
  ...
  Future<AssetId> _writeOutput(String path, String content) async {
    final id = _output(path);
    await writeAsString(id, content);
    return id;
  }

  // write a spec at path and format it.
  Future<AssetId> emit(String path, Spec spec) async {
    final content = spec.accept(emitter).toString();
    return writeAndFormat(path, content);
  }

  // the main function used to output files on disk
  Future<AssetId> writeAndFormat(String path, String content) async {
