...
class MyStateMachine extends StateMachine<Event, State> {
  MyStateMachine() : super(InitialState()) {
    define<InitialState>(($) => $
      ..onEnter((OtherState state) { /** onEnter side effect */ })
      ..onChange((OtherState state, OtherState nextState) { /** onChange side effect */ })
      ..onExit((OtherState state) { /** onEnter side effect */ })
      ..on<SomeEvent>((SomeEvent event, OtherState state) => OtherState()));
