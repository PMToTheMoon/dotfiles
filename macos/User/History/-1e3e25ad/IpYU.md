...
state_machine_bloc is an extension to the bloc state management library which provide `StateMachine` utility to create blocs that behave like finite state machines. Event routing and filtering are done under the hood for you so you can focus on building state machine logic.
