...
state_machine_bloc is an extension to the bloc state management library which provide `StateMachine` utility class to create finite states machines blocs. Event routing and filtering are done under the hood for you so you can focus on building state machine logic.
