...
class MyStateMachine extends StateMachine<Event, State> {
  MyStateMachine() : super(InitialState()) {
    define<InitialState>(($) => $
      ..onEnter((InitialState state) { /** onEnter side effect */ })
      ..onChange((InitialState state, InitialState nextState) { /** onChange side effect */ })
      ..onExit((InitialState state) { /** onExit side effect */ })
