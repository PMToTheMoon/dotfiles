...
class _SignInPageState extends State<_SignInPage> {
  ...
  late final _controller =
      _SignInFormController(onChanged: _onBlocStateChanged);

  bool _enableSubmitButton;

  void _onFormDataChanged() {
    final _enableSubmitButton = _controller.firstName?.isNotEmpty == true &&
  ...
  void _onBlocStateChanged(BuildContext ctx, SignInState state) {
    if (state is SignInInvalidForm) {
      showError(ctx, message: ctx.l10n.error_indicator_message);
    }
    // if (state is SignInWaitingCodeValidation) {
    //   PinCodePage.open(ctx);
    // }
  }
                      ...
                      return SubmitButton(
                        ...
                        onSubmit: () {
                          PinCodePage.open(
                            context,
                            phoneNumber:
                                _controller.phoneNumber?.completeNumber ?? "",
                          );
                        }, //_onSubmitForm,
                        loading: state is SignInSubmittingForm,
                      );
                    },
                  ),
                ),
