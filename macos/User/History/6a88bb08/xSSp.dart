...
class AuthRepository {
  ...
  Future<void> sendSmsCode({
    ...
    final data = phoneNumber.trim();
    _urbanDriver.sendSmsVerificationCode(data);
  }

  Future<void> signin({
    required SignInFormData data,
  }) async {
    try {
      final _data = data.trim();
      final token = await _urbanDriver.signin(SignInRequest(
      ...
      await Future.wait([
        _setCachedEmail(_data.email),
        _setCachedPassword(_data.password),
        _setCachedToken(token),
      ]);
      _statusStream.add(AuthStatus.authenticated);
    } on Unauthorized {
