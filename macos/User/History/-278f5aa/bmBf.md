...
# field copy from a model to another
+ manipulation: exclude, make nullable or not...
+ cobject composition

## REQUIREMENT:
Must be able to access data from bellow (ex: barrel, json factories gathering)
-> access after child build (taht way we can refer to them their file)
something like `MetaData<MetaObject>`

Must be able to access data from above (ex: repository)
-> access from build context in seed ?

## It's very important to have a robust logging/error reporting system
