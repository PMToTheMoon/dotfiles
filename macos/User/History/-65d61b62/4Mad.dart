...
mixin AnimatedListDiff<T> on StatefulWidget {
  List<T> get items;
}

mixin AnimatedListDiffState<T> on State<AnimatedListDiff<T>> {
  late List<T> _effectiveEducations = widget.items;
  ...
  void didUpdateWidget(old) {
    ...
    if (widget.items.length != _effectiveEducations.length) {
      final insertedIndexes = widget.items
          .whereNot((e) => _effectiveEducations.contains(e))
          .mapIndexed((index, element) => index);
      final removedIndexes = _effectiveEducations.mapIndexed((index, element) {
      ...
      for (final r in removedIndexes) {
        final item = _effectiveEducations[r];
    ...
    _effectiveEducations = widget.educations;
  }
}
...
class _CandidateEducationsListViewState
  ...
  late List<Education> _effectiveEducations = widget.educations;
  ...
  void didUpdateWidget(old) {
    ...
    if (widget.educations.length != _effectiveEducations.length) {
      final insertedIndexes = widget.educations
          .whereNot((e) => _effectiveEducations.contains(e))
          .mapIndexed((index, element) => index);
      final removedIndexes = _effectiveEducations.mapIndexed((index, element) {
        if (widget.educations.contains(element)) return null;
        return index;
      }).whereNotNull();
      for (final i in insertedIndexes) {
        _animatedList.currentState?.insertItem(i);
      ...
      for (final r in removedIndexes) {
        ...
        _animatedList.currentState?.removeItem(
          r,
          (context, animation) => UnclippedSizeTransition(
