...
class AnimatedDeltaList<T> extends StatefulWidget {
  const AnimatedDeltaList({super.key, required this.items});

  final List<T> items;
  final InsertBuilder<T> insertBuilder;
  final RemoveBuilder<T> removerBuilder;

  @override
  State<StatefulWidget> createState() => AnimatedDeltaListState();
}

class AnimatedListDiffState<T> extends State<AnimatedDeltaList<T>> {
    ...
    return AnimatedList(
      ...
      itemBuilder: (context, index, animation) => UnclippedSizeTransition(
        ...
        child: UnfoldableEducationCard(
          key: ValueKey(_effectiveItems[index].id),
          education: _effectiveItems[index],
        ),
      ),
    );
  }
