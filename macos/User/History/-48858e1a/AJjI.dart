...
class FileBasedAssetReader extends AssetReader
  ...
  Stream<AssetId> findAssets(Glob glob, {String? package}) {
    ...
    return glob
        ...
        .cast<File>()
        .map((file) => _fileToAssetId(file, packageGraph.root, packageNode));
  }
  ...
  @override
  String packagePathTo(AssetId id) => _packageFilePathFor(id, packageGraph);
}

/// Creates an [AssetId] for [file], which is a part of [packageNode].
AssetId _fileToAssetId(
  File file,
  PackageNode root,
  PackageNode? packageNode,
) {
  var filePath = p.normalize(file.absolute.path);
  if (packageNode == null || packageNode.isRoot) {
    var relativePath = p.relative(filePath, from: root.path);
    return AssetId.input(relativePath);
  } else {
    return AssetId.package(filePath);
  }
}
...
class FileBasedAssetWriter implements RunnerAssetWriter {
  ...
  Future delete(AssetId id) {
    if (p.isWithin(packageGraph.root.path, id.path) != packageGraph.root.path) {
...
String _filePathFor(AssetId id, PackageGraph packageGraph) {
  if (id.type == AssetType.input) {
    return id.path;
  } else if (id.type == AssetType.package) {
    return p.join(packageGraph.root.path, id.path);
  } else {
    return id.path;
  }
}
...
/// Returns the path to [id] for a given [packageGraph].
String _packageFilePathFor(AssetId id, PackageGraph packageGraph) {
  String path = _filePathFor(id, packageGraph);
  path = p.relative(path, from: packageGraph.root.path);
  path = p.normalize(path);
  return 'package:${packageGraph.root.name}/$path';
}
