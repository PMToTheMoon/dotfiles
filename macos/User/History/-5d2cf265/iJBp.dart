...
class UnionType {
  ...
  Constructor toConstructor(String className, String? prefix, String? suffix) =>
      Constructor(
        (c) => c
          ..name = name
          ...
          ..factory = true
          ..redirect = refer(
              '${prefix ?? ''}${name?.pascalCase ?? className}${suffix ?? ''}'),
      );
}
...
class Union extends MetaObject<UnionMetaData> {
  ...
  Future<UnionMetaData> build(BuildStep step) async {
    ...
    final constructors = types
        .map((t) => t.toConstructor(className, typePrefix, typeSuffix))
        .toList();
    if (serializable) {
      constructors.add(
        Constructor(
    ...
    final unionFileContent = Library(
      (l) => l
        ..directives.addAll([
          ...
          if (step.isPartOf == false && serializable)
            Directive.part(jsonSerializableFileName),
        ])
        ..body.add(union),
    ...
    return UnionMetaData(
      ...
      types: {
        for (final c in constructors)
          c.name ?? 'default':
              c.redirect?.type.accept(step.emitter).toString() ?? 'private',
      },
    );
