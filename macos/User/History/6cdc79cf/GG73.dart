...
class _PaymentMethodsPageBody extends StatelessWidget {
  ...
  void _onAddNewPayment(BuildContext context) async {
    final result = await NewPaymentMethodPage.open(context);
    if (result == NewPaymentMethodResult.success) {
      context
          .read<PaymentMethodsBloc>()
          .add(const PaymentMethodsEvent.newPaymentMethodAdded());
    } else if (result == NewPaymentMethodResult.failure) {
      showErrorDialog(
        context,
        message: context.l10n.new_payment_method_creation_failed,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
      ...
      const Space4.h(),
      Button.menu(
        context.l10n.new_payment_method_button,
        () => _onAddNewPayment(context),
        icon: Icons.add,
