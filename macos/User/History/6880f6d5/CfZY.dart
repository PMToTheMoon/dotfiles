...
void main(List<String> args) {
  group("nested state lifecycle test", () {
    ...
    test("nested state onEnter called at initialization id it's the initial state", () async {
    ...
    test("nested state onExit called when entering sub state", body)
    test("nested state onChange called when transiting to same sub state", body)

    test("parent's onEnter called when transiting to one of its sub states", body)
    test("parent's onChange called when transiting to another of its sub states", body)
    test("parent's onExit called when transiting to a state that is not one of its child", body)
  });

  group("nested state event test", () {
    test("child state received event", body)
    test("parent state received event", body)
  });
}
