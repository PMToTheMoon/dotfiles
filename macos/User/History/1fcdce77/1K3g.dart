...
class ComplementFormCard extends StatelessWidget {
  ...
  void _onSubmitted(BuildContext context, NewComplementFormSnapshot snapshot) {
  ...
  void _onCloseComplementSubmitted(
    BuildContext context,
    CloseComplementFormSnapshot snapshot,
  ) {
    context
        .read<ObservationsBloc>()
        .add(ObservationsEvent.closeComplementSubmitted(
          title: snapshot.title,
          message: snapshot.patientReason,
        ));
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (complementType == ComplementType.close)
      ...
      card = CloseComplementFormCard(
        onFormSubmitted: (snapshot) =>
            _onCloseComplementSubmitted(context, snapshot),
      );
    else {
      card = NewComplementFormCard(
        onFormSubmitted: (snapshot) => _onSubmitted(context, snapshot),
      );
    }
