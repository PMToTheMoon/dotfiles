...
class RapportFormCard extends StatelessWidget {
  ...
  void onConsultationSubmitted(
      BuildContext context, NewGeneralRapportFormSnapshot snapshot) {
    ...
    final rapport = RapportGeneral(
      id: hub.effectiveRecord.rapportGeneral?.id ?? 0,
      recordId: hub.effectiveRecord.id,
      providerId: userId,
      synthesis: snapshot.hiddenReason,
      comment: snapshot.rapport,
  ...
  void onTeleExpertiseSubmitted(
    ...
    final rapport = RapportGeneral(
      ...
      teleExpertiseComment: snapshot.rapport,
      teletransmissionAutre: false,
    );
    context
        .read<ObservationsBloc>()
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: NewGeneralRapportCard(
        onFormSubmitted: (snapshot) => onFormSubmitted(context, snapshot),
        request: RequestCard(
          from: hub.effectiveRecord.doctorFirstName ?? 'unknown',
