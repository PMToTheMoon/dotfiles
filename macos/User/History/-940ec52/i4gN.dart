...
import 'package:zanalys/root/root.dart';
...
class _ZanalysAppState extends State<ZanalysApp> {
  ...
  // This is more or less legacy code from first dev
  // we need a more robust system that support:
  // deep links with different kind of notifications
  // localized messages
  ...
  Future<void> _setupPushNotificationsHandler() async {
    ...
    FirebaseMessaging.instance.onTokenRefresh.listen((token) async {
      var authToken = await AppServices.storage.getBearerToken();
      if (authToken != null) {
  ...
  void _handleMessage(RemoteMessage message) {
    if (AppServices.auth.status == AuthStatus.authenticated) {
      handlePushNotification(_navigator, message.data);
    } else {
      _scaffoldMessenger
        ..hideCurrentSnackBar()
        ..showSnackBar(
          // TODO: find a way to get a localized error message.
          // This problem should be addressed while refactoring notification
          const SnackBar(
            content: Text(
