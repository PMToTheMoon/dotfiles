    ...
    return Actions(
      actions: {
        ToggleUnfoldableItem: CallbackAction<ToggleUnfoldableItem>(
          onInvoke: (intent) => _controller.toggle(widget.key!),
        ),
        FoldUnfoldableItem: CallbackAction<FoldUnfoldableItem>(
          onInvoke: (intent) => _controller.fold(widget.key!),
        ),
        UnfoldUnfoldableItem: CallbackAction<UnfoldUnfoldableItem>(
          onInvoke: (intent) => _controller.unfold(widget.key!),
        ),
      },
...
class UnfoldableListCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return TbtHoverCard(
      ...
      //   Actions.invoke(context, UnfoldUnfoldableItem());
      // },
