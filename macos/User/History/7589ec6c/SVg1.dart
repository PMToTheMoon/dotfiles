...
Future<List<GeneratorApplication>> generateAndRun(
  ...
  while (tryCount < 2 && !succeeded) {
    ...
    try {
      ...
      final buildScriptPubspec = File(scriptPubspecLocation);
      var oldContents = '';
      ...
      if (newContents != oldContents) {
        ...
        buildScriptPubspec
          ..createSync(recursive: true)
          ..writeAsStringSync("""
name: bonemeal_build_script
dependencies:
  bonemeal_runner:
    path: "/Users/pm/Project/bonemeal/packages/bonemeal_runner"
""");
      }
    ...
    try {
      await Isolate.spawnUri(Uri.file(p.absolute(scriptKernelLocation)), args,
          ...
          errorsAreFatal: true,
          packageConfig: Uri.file(p.absolute(scriptPubspecLocation)),
